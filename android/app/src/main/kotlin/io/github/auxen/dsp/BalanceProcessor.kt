 ...
 * moving toward one side attenuates the OTHER side rather than boosting the
 * favored one, so balance never adds headroom pressure to the chain.
...
class BalanceProcessor : BaseAudioProcessor() {
    ...
    override fun queueInput(inputBuffer: ByteBuffer) {
        val s = state
        val active = s.enabled && s.balance != 0f && channelCount >= 2
        val leftGain = min(1f, 1f - s.balance)
        val rightGain = min(1f, 1f + s.balance)
